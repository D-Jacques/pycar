...
app.config.from_mapping(
    SECRET_KEY="dev", DATABASE=os.path.join(app.instance_path, "pycar.db")
)
...
def register():
    ...
    if request.method == "POST":
        ...
        if error is None:
            ...
            db_connect.commit()
            flash(
                "Le compte est désormais inscrit, l'utilisateur peut se connecter avec ce compte"
            )
        else:
            flash(error)
...
def connection():
    if request.method == "POST":
        ...
        checkUser = db_user.execute(
            "SELECT * FROM pycar_user WHERE username = ?", (username,)
        ).fetchone()
        ...
        if checkUser is None:
            ...
            error = (
                "Erreur : Le mot de passe renseigné est incorrect pour cet utilisateur"
            )
        if error is None:
            session.clear()
...
def page_profil():
    ...
    user_datas = db_user.execute(
        "SELECT user_mail FROM pycar_user WHERE id = ?", (session["id"],)
    ).fetchone()
...
def change_mail():
    ...
    if request.method == "POST":
        ...
        if mail_user == new_mail_user:
            ...
            error = (
                "Vous devez rentrer votre nouvelle adresse mail (minimum 10 caractères)"
            )
        elif new_mail_user != new_mail_check:
            error = (
                "Vous devez rentrer la même adresse mail dans les deux derniers champs"
            )
        elif (
            db_user.execute(
                "SELECT * FROM pycar_user WHERE user_mail = ?", (new_mail_user,)
            ).fetchone()
            is not None
        ):
        ...
        if error is not None:
            ...
            db_user.execute(
                "UPDATE pycar_user SET user_mail = ?" " WHERE id = ?",
                (
                    new_mail_user,
                    session["id"],
                ),
...
def change_password():
    ...
    if request.method == "POST":
        ...
        user_datas = db_user.execute(
            "SELECT user_password FROM pycar_user WHERE id = ?", (session["id"],)
        ).fetchone()
        ...
        if password_user == new_password_user:
            ...
            error = (
                "Vous devez rentrer le même mot de passe dans les deux dernier champs"
            )
        ...
        if error is not None:
            ...
            db_user.execute(
                "UPDATE pycar_user SET user_password = ?" "WHERE id = ?",
...
def car_board():
    db_cars = db.get_db()
    cars_data = db_cars.execute("SELECT * FROM pycar_cars").fetchall()
...
def car_modification(id_car=None):
    ...
    car_selected = db_cars.execute(
        "SELECT * FROM pycar_cars WHERE id = ?", (id_car,)
    ).fetchone()
...
def car_delete(id_car):
    ...
    if car_selected is None:
        ...
        db_cars.execute("DELETE FROM pycar_cars WHERE id = ?", (id_car,))
        db_cars.commit()
...
def car_create_sheet(id_car):
    ...
    cars_data = db_cars.execute(
        "SELECT car_name FROM pycar_cars WHERE id = ?", (id_car,)
    ).fetchone()
    ...
    if os.path.isfile("car_sheets/" + cars_data["car_name"] + "-" + id_car + ".txt"):
        with open(
            "car_sheets/" + cars_data["car_name"] + "-" + id_car + ".txt", "r+"
        ) as car_sheet_file:
    ...
    if request.method == "POST":
        ...
        with open(
            "car_sheets/" + cars_data["car_name"] + "-" + id_car + ".txt", "w"
        ) as car_sheet_file:
    ...
    return render_template(
        "sheet_creation.html", id_car=id_car, file_content=file_content
    )
